...
# new_nums = list(map(squareFunc, nums))
# new_grades = list(map(toGrade, grades))
...
# new_grades = list(map(toGrade, sorted_grades))
# print(new_grades)
...
def simplify(quake):
    ...
    return {
        "place": q['place'],
        "magnitude": q['mag'],
        "date": str(datetime.date.fromtimestamp(q['time']/1000))
    }
